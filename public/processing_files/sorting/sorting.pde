...
void processAnswer(int userInput, int[]blockValuesInput) {    //Called when User submit their inputs
  blockNum = userInput;
  blockValues = blockValuesInput;
...
class Button {
  ...
  void drawBtn() {
    ...
    textSize(fontSize);
    textAlign(CENTER, CENTER);
    text(tag, pos.x+size.x/2, pos.y+size.y/2);
  }
}